...
const updateResultsList = () => {
  ...
  units.forEach((unit) => {
    if (unit !== unitToConvert.value) {
      ...
      const newServings = Number(numberOfServings.value);
      const convertedQuantity = processIngredient(quantity, baseUnit, unit, newServings);
      const listItem = `${ingredient}: ${convertedQuantity} ${unit}`;
